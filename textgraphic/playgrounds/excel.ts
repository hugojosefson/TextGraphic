...
const sleep = () => new Promise((resolve) => setTimeout(resolve, WAIT_MS));
...
function createCell(text: string, active: boolean, styler: StyleRenderer) {
  const border =
    (active ? TextGraphic.Presets.HeavyBox : TextGraphic.Presets.LightBox);

  const box = TextGraphic.ShapeView.initWithText(text, 0, styler, {
    bold: true,
  })
...
function renderTable(activeColumn: number, styler: StyleRenderer) {
  const canvasWidth = (((CELL_WIDTH - 1) * TABLE_COLUMNS) + 2) + LEFT_PADDING;
  const canvasHeight = (((CELL_HEIGHT - 1) * TABLE_ROWS) + 1) +
    1 * PADDING_VERTICALLY;
  const tableCanvas = new TextGraphic.CanvasView(
    canvasWidth,
    canvasHeight,
    styler,
  );
  ...
  for (let row = 0; row < TABLE_ROWS; row++) {
    for (let column = 0; column < TABLE_COLUMNS; column++) {
      const active = row === 1 && column === activeColumn;
      const x = (column * (CELL_WIDTH - 1)) + 1 + LEFT_PADDING;
      const y = (row * (CELL_HEIGHT - 1)) + PADDING_VERTICALLY;
      const text = `${ALPHABET[column]}${row + 1}`;
      const cell = createCell(text, active, styler);
