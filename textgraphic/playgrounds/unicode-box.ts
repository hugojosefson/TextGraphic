...
function createSample() {
  const background = new TextGraphic.CanvasView(7, 4, renderer);

  const sampleBox1 = TextGraphic.ShapeView.initBlankRectangle(3, 1, renderer)
    .frame(TextGraphic.Presets.LightBox);

  const sampleBox2 = TextGraphic.ShapeView.initBlankRectangle(3, 1, renderer)
...
const normalSample = createSample();
const tunnedSample = createSample()
  .fineTuneBoxIntersections();
const arrow = new TextGraphic.LineView("becomes", renderer, {});
const spacing = 3;

const container = new TextGraphic.CanvasView(
  2 * (spacing + normalSample.width) + arrow.width,
  normalSample.height,
  renderer,
);

container.add(normalSample, 0, 0, 0);
...
container.add(tunnedSample, container.width - tunnedSample.width, 0, 0);
