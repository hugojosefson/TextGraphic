...
export function parseTimpaniToANSITerminal(
  timpaniCode: string,
): FlattenANSITerminalNode[] {
  const timpaniTree = parseTimpaniMarkup(timpaniCode);
  const flattenANSIArray = convertTimpaniTreeToFLattenANSITerminal(timpaniTree);
  return flattenANSIArray;
}

export function compileTimpaniToANSITerminalSequence(
  timpaniCode: string,
): string {
  const parsedANSITerminal = parseTimpaniToANSITerminal(timpaniCode);
  let terminalOutput = EMPTY_STRING;
...
class TimpaniLanguageParser {
  ...
  constructor(code: string) {
    ...
    this.#pointer = 0;
    this.#results = new Array<TimpaniToken>();
  ...
  private parseOneCharSignedGrammar(
    sign: string,
    type: TimpaniStyleType,
  ): TimpaniToken {
...
function convertTimpaniTreeToFLattenANSITerminal(
  ...
  //
  let result = new Array<FlattenANSITerminalNode>();

  for (const rootPart of tree) {
    const terminalTag = generateANSITerminalStartingTagFromTimpaniCode(
      rootPart.type,
      parentANSITag,
    );
    ...
    for (const subPart of rootPart.parts) {
      if (typeof subPart === "string") {
        ...
        const sequence = convertTimpaniTreeToFLattenANSITerminal(
          [subPart],
          terminalTag,
        );
