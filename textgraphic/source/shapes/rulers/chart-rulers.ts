...
export function createChartRuler<
...
>(
  ...
  //
  const settings = fixSettings(inputSettings);
...
function fixSettings(settings: CharRulerSettings): FixedCharRulerSettings {
  return {
    ...
    size: Math.floor(settings.size),
    chars: settings.chars,
    verticalGutterSize: settings.verticalGutterSize
      ? Math.max(
        Math.floor(settings.verticalGutterSize),
        Math.ceil(Math.log10(settings.size)) + 1,
      )
      : Math.ceil(Math.log10(settings.size)) + 1,
    hideNumbers: settings.hideNumbers || false,
    unit: settings.unit ? Math.floor(settings.unit) : 10,
    unitSize: settings.unitSize ? settings.unitSize : 1,
  };
}
...
function createHorizontalChartRuler<
  ColorType extends PortableColor,
  EnvironmentStyleSettings extends PortableStyle<any>,
>(
  ...
  //
  const rulerLine = createHorizontalRulerLine(settings);
  ...
  //
  const numberLine = createHorizontalRulerNumberLine(settings);

  const lines = isDirectionUp
    ? [rulerLine, numberLine]
    : [numberLine, rulerLine];
  const baseline = isDirectionUp ? 0 : 1;
...
>(
  ...
  const { hideNumbers, size, chars, verticalGutterSize, unit, unitSize } =
    settings;
  const { originChar, middleChar, separatorChar } = chars;
  const lines = new Array<string>(size);

  // in case of hide numbers
  if (hideNumbers) {
    lines[0] = originChar;
    for (let i = 1; i < size; i++) {
      lines[i] = i % unit == unit - 1 ? separatorChar : middleChar;
    }
  ...
  function createNumberLabel(no: string) {
    const paddingLength = verticalGutterSize - 1 - no.length;
    return facingLeft
      ? WHITE_SPACE_CHARACTER + no + WHITE_SPACE_CHARACTER.repeat(paddingLength)
      : WHITE_SPACE_CHARACTER.repeat(paddingLength) + no +
        WHITE_SPACE_CHARACTER;
  }
  ...
  const middleLineTemplate = facingLeft
  ...
  function createMiddleLine(i: number) {
    const label = createNumberLabel(((i * unitSize) + 1).toString());
    return facingLeft ? separatorChar + label : label + separatorChar;
  }

  for (let i = 1; i < size; i++) {
    lines[i] = i % unit == unit - 1 ? createMiddleLine(i) : middleLineTemplate;
  }
...
function createHorizontalRulerNumberLine(
  settings: FixedCharRulerSettings,
): string {
  //
  const { size, unit, unitSize } = settings;
  const numberLineSections = Math.floor(size / unit);
  const numberLineArray = new Array<string>(numberLineSections);

  for (let i = 0; i < numberLineSections; i++) {
    const unitNumberText = (unit * unitSize * (i + 1)).toString();
    if (i === 0) {
      numberLineArray[i] = "1" +
        WHITE_SPACE_CHARACTER.repeat(unit - unitNumberText.length) +
        unitNumberText;
    } else {
      numberLineArray[i] =
        WHITE_SPACE_CHARACTER.repeat(unit - unitNumberText.length) +
...
function createHorizontalRulerLine(settings: FixedCharRulerSettings): string {
  //
  const { chars, size: width, unit } = settings;
  const { originChar, middleChar, separatorChar } = chars;

  let rulerLine = originChar;
