...
>(
  ...
  const graphWidth = 2 / horizontalZoom;
  const graphHeight = 2 / verticalZoom;
  const deltaOriginX = graphWidth / 2 - originX;
  const deltaOriginY = graphHeight / 2 - originY;

  for (let row = 0; row < height; row++) {
    const y = (((height - row) / height) * graphHeight) - deltaOriginY;
    const line = new Array<string>(width);

    for (let column = 0; column < width; column++) {
      const x = (column / width) * graphWidth - deltaOriginX;
      line[column] = formula(x, y, graphWidth, graphHeight)
        ? character
        : WHITE_SPACE_CHARACTER;
    }
    lines[row] = line.join("");
...
function checkAndCompleteGraphSettings<
  ColorType extends PortableColor,
  EnvironmentStyleSettings extends PortableStyle<ColorType>,
>(
  settings: GraphSettings<ColorType, EnvironmentStyleSettings>,
  ...
  // character
  let drawingCharacter = BLOCK_CHARACTER;
  if (character) {
