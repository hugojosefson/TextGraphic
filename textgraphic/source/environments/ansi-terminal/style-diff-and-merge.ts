...
export function terminalStylesEquals(
  ...
  //
  let allElementsAreEquals = true;

  const textColor = a.color == b.color;
  allElementsAreEquals = allElementsAreEquals && textColor;

  const backgroundColor = a.backgroundColor == b.backgroundColor;
...
export function generateStartingANSITerminalEscapeSequenceOfTerminalStyling(
  ...
  //
  const bold = style.bold ? EscapeSequences.Bold : EMPTY_STRING;
  const italic = style.italic ? EscapeSequences.Italic : EMPTY_STRING;
  const reversed = style.reversed ? EscapeSequences.Reversed : EMPTY_STRING;
  const blink = style.blink ? EscapeSequences.Blink : EMPTY_STRING;
  const underline = style.underline ? EscapeSequences.Underline : EMPTY_STRING;
  const dim = style.dim ? EscapeSequences.Dim : EMPTY_STRING;

  const backgroundColor = convertPortableColorToANSITerminalColor(
    style.backgroundColor,
    true,
  );
  const foregroundColor = convertPortableColorToANSITerminalColor(
    style.color,
    false,
  );
