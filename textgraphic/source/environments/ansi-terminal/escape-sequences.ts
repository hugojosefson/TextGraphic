...
export const Reset = formANSITerminalEscapeSequence("0");
export const Bold = formANSITerminalEscapeSequence("1");
export const Dim = formANSITerminalEscapeSequence("2");
export const Italic = formANSITerminalEscapeSequence("3");
export const Underline = formANSITerminalEscapeSequence("4");
export const Blink = formANSITerminalEscapeSequence("5");
export const Reversed = formANSITerminalEscapeSequence("7");
export const Hidden = formANSITerminalEscapeSequence("8");

//
// ─── FOREGROUND COLOR LUT ───────────────────────────────────────────────────────
//

export function getForegroundColorEscapeSequenceForLabledColor(
  color: PortableLabeledColors,
): string {
  switch (color) {
    case "black":
...
export function getBackgroundColorEscapeSequenceForLabledColor(
  color: PortableLabeledColors,
): string {
  switch (color) {
    case "black":
...
export function formatColorTo24BitANSITerminalColor(
  color: RGBColor,
  ...
  const backgroundForegroundSelection = isBackground ? "48" : "38";

  const colorCode = formANSITerminalEscapeSequence(
    backgroundForegroundSelection,
    "2",
    color.red.toString(),
...
export function formatMiddleLUTColorTo8BitANSITerminalColor(
  color: RGBColor,
): string {
  const red = Math.floor(color.red / (256 / 36));
  const green = Math.floor(color.green / (256 / 6));
  const blue = Math.floor(color.blue / (256 / 6));

  const colorCode = (red + green + blue).toString();
  const escapeSequence = formANSITerminalEscapeSequence("38", "5", colorCode);
