...
export class SVGStyleRenderer
  implements StyleRendererProtocol<PortableColor, SVGStyleSettings> {
  //
  // ─── STORAGE ─────────────────────────────────────────────────────
  //
  ...
  constructor(optimizeCSS: boolean, globalSettings: GlobalSVGSettings) {
    this.#fontFamily = globalSettings.fontFamily;
    this.#fontSize = globalSettings.fontSize;
    this.#optimizer = new CSSStyleOptimizer(optimizeCSS);
  }
  ...
  public renderLeftStylingInfo(style: SVGStyleSettings): string {
    const css = convertSVGSettingsToInlineCSS(style);
  ...
  public wrapRootLinesAndFinalizeRender(
    ...
    // header
    renderedParts[0] = this.generateSVGHeader(width, lines.length);

    // body
    for (let i = 0; i <= lines.length; i++) {
      const y = ((i + 1) * this.#fontSize) - 3;
      renderedParts[i + 1] = `${INDENTATION}<text x="0" y="${y}px">${
        lines[i]
      }</text>`;
    }

    // footer
    renderedParts[lines.length + 1] = "</svg>";
  ...
  private generateSVGHeader(width: number, height: number) {
    const TextGraphicComment = `<!-- Generated by Pouya's TextGraphic -->\n`;

    const TextGraphicAreaClassName = "textgraphic-svg-area";

    const widthInPixels = width * 0.6 * this.#fontSize;
    const heightInPixels = height * this.#fontSize;
    const SVGHeader =
    ...
    const styleTag = this.#optimizer.generateHeaderStyleTag(additionalCSS);
