...
export type StyleSerializer = (style: string) => string;
...
export class CSSStyleOptimizer {
  ...
  constructor(optimize: boolean) {
    this.#classRegistry = {};
    this.#generatedClassNames = new Set<string>();
  ...
  private getClassNameByStyle(serializedCSSStyle: string) {
    if (serializedCSSStyle in this.#classRegistry) {
      ...
      this.#classRegistry[serializedCSSStyle] = newClassName;
      return newClassName;
    }
  }
  ...
  public generateHeaderStyleTag(additionalCSSElementStyle: string): string {
    ...
    //
    const addition = additionalCSSElementStyle === "" ? 0 : 1;
    const classRows = new Array<string>(
      this.#generatedClassNames.size + addition,
    );

    //
    if (addition === 1) {
      classRows[0] = INDENTATION + INDENTATION + additionalCSSElementStyle;
    }
    ...
    if (this.#optimize) {
      ...
      for (const serializedStyle of Object.keys(this.#classRegistry)) {
        const className = this.#classRegistry[serializedStyle];
        classRows[offset] =
    ...
    //
    const styleTagHead = `\n${INDENTATION}<style>\n`;
    const styleTagContent = classRows.join("\n");
    const styleTagTail = `\n${INDENTATION}</style>`;
...
function generateRandomCharacter() {
  const startPoint = 97;
  const stopPoint = 122;
  const codePoint = startPoint + Math.floor(
    Math.random() * (
