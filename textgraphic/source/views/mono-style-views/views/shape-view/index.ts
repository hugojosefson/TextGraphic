...
> implements StylableViewProtocol<ColorType, EnvironmentStyleSettings> {
  //
  // ─── STORAGE ─────────────────────────────────────────────────────
  //
  ...
  readonly styleRenderer: StyleRendererProtocol<
    ColorType,
    EnvironmentStyleSettings
  >;
  ...
  constructor(
    ...
    if (
      typeof baseline !== "number" || baseline < 0 || baseline >= lines.length
    ) {
      throw new Error(
    ...
    this.#style = styleRenderer.margeNewStyleOptionsWithPreviosuStyleState(
      styleRenderer.defaultStyle,
      style,
    );
    this.#leftStylingInfoCache = styleRenderer.renderLeftStylingInfo(
      this.#style,
    );
    this.#rightStylingInfoCache = styleRenderer.renderRightStylingInfo(
      this.#style,
    );
  }

  static initWithSpaceCheck<
    ColorType extends PortableColor,
    StyleSettings extends PortableStyle<ColorType>,
  >(
    lines: string[],
    baseLine: number,
    ...
    //
    const unifiedLines = unifyLineSpaces(lines);
  ...
  >(
    ...
    //
    const lines = breakStringIntoLines(text);
    const unifiedLines = unifyLineSpaces(lines);
    return new ShapeView(unifiedLines, baseLine, styler, initialStyle, true);
  }
  ...
  static initEmptyBox<
    ColorType extends PortableColor,
    StyleSettings extends PortableStyle<any>,
  >(
    styler: StyleRendererProtocol<ColorType, StyleSettings>,
  ) {
  ...
  >(
    ...
    //
    const backgroundLine = backgroundChar.repeat(width);
  ...
  private applyNewStyle(
    sourceStyle: EnvironmentStyleSettings,
    changes: Partial<EnvironmentStyleSettings>,
  ) {
    //
    this.#style = this.styleRenderer.margeNewStyleOptionsWithPreviosuStyleState(
      sourceStyle,
      changes,
    );
    this.#leftStylingInfoCache = this.styleRenderer.renderLeftStylingInfo(
      this.#style,
    );
    this.#rightStylingInfoCache = this.styleRenderer.renderRightStylingInfo(
      this.#style,
    );
  }
  ...
  public get styledForm(): string {
    const styledLines = new Array<string>(this.height);
    const { encodeCharacterForStyledRender } = this.styleRenderer;
    const leftInfo = this.#leftStylingInfoCache;
    const rightInfo = this.#rightStylingInfoCache;

    for (let row = 0; row < this.height; row++) {
      const encodedLine = replaceCharacters(
        this.lines[row],
        encodeCharacterForStyledRender,
      );
      styledLines[row] = leftInfo + encodedLine + rightInfo;
    }
  ...
  public getCharAtRelativePosition(
    left: number,
    top: number,
    x: number,
    y: number,
