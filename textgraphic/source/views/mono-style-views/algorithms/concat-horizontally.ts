...
export function concatMonoStyledViewsHorizontally<
...
>(
  ...
  for (const box of boxes) {
    ...
    if ((box.height - 1 - box.baseline) > heightBelowNewBaseline) {
      heightBelowNewBaseline = box.height - 1 - box.baseline;
    }
  }
  ...
  // boxesWithPadding
  const boxesWithJoiner = insertJoinersInBetweenArrayItems(boxes, joiner);
  ...
  const newLines = new Array<string>(rows);

  for (let row = 0; row < rows; row++) {
    const lineColumns = new Array<string>(columns);
    for (let column = 0; column < columns; column++) {
      lineColumns[column] = boxesWithAppropriatePaddings[column].lines[row];
    }
    newLines[row] = lineColumns.join(EMPTY_STRING);
  }
