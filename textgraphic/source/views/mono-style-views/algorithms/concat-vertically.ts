...
export function concatMonoStyledViewsVertically<
...
>(
  ...
  const resultWidth = Math.max(...boxes.map((box) => box.width));
  const lines = new Array<string>();

  for (const box of boxes) {
    const centeredBox = box.centerToBoundaryBox(resultWidth, box.height);
