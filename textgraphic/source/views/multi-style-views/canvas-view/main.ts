...
export interface CanvasChildrenProfile<
...
> {
  ...
  child: ViewProtocol<
    ...
    StyleRendererProtocol<ColorType, EnvironmentStyleSettings>
  >;
}
...
> implements
  ...
  readonly styleRenderer: StyleRendererProtocol<
    ...
    EnvironmentStyleSettings
  >;
  readonly #children: CanvasChildrenProfile<
    ColorType,
    EnvironmentStyleSettings
  >[];
  ...
  constructor(
    ...
    this.height = height;
    this.width = width;
    const baseStyle = styleRenderer.renderLeftStylingInfo(
      styleRenderer.defaultStyle,
    );
    this.screen = new VirtualScreen(this.width, this.height, baseStyle);
  ...
  static initWithBackground<
    ColorType extends PortableColor,
    EnvironmentStyleSettings extends PortableStyle<ColorType>,
  >(
    background: ViewProtocol<
      ColorType,
      EnvironmentStyleSettings,
      StyleRendererProtocol<ColorType, EnvironmentStyleSettings>
    >,
  ...
  public *getChildren(): Generator<
    CanvasChildrenProfile<ColorType, EnvironmentStyleSettings>,
    null
  > {
  ...
  private updatePortionOfScreenMatrix(
    left: number,
    top: number,
    width: number,
    height: number,
    ...
    for (let x = left; x < width + left; x++) {
      for (let y = top; y < height + top; y++) {
        if (x >= 0 && x < this.width && y >= 0 && y < this.height) {
          this.screen.write(x, y, this.rayTrace(0, 0, x, y));
        }
      }
    }
  }
  ...
  public fineTuneBoxIntersections(): CanvasView<
    ColorType,
    EnvironmentStyleSettings
  > {
    fineTuneUnicodeBoxForLayeredCanvas(
  ...
  public fineTuneBoxIntersectionsInSelectedArea(
    ...
    if (startX < 0 || startX >= this.width) {
      throw new Error(
        `Area's Starting X point should be in bounds [0 - ${
          this.width - 1
        }] but found: ${startX}.`,
      );
    }
    ...
    if (startY < 0 || startY >= this.height) {
      throw new Error(
        `Area's Starting Y point should be in bounds [0 - ${
          this.height - 1
        }] but found: ${startY}.`,
      );
    }
    ...
    if (endX < 0 || endX >= this.width) {
      throw new Error(
        `Area's Ending X point should be in bounds [0 - ${
          this.width - 1
        }] but found: ${endX}.`,
      );
    }
    ...
    if (endY < 0 || endY >= this.height) {
      throw new Error(
        `Area's Ending Y point should be in bounds [0 - ${
          this.height - 1
        }] but found: ${endY}.`,
      );
    }
  ...
  public centerToBoundaryBox(
    ...
    //
    return centerViewProtocolToBoundaryBox(this, width, height) as CanvasView<
      ColorType,
      EnvironmentStyleSettings
    >;
  }
  ...
  public createCrop(x: number, y: number, width: number, height: number) {
    const cropView = new CanvasView(width, height, this.styleRenderer);
  ...
  public sliceHorizontally(
    ...
    //
    const leftSlice = this.createCrop(0, 0, x, this.height);
    const rightSlice = this.createCrop(x, 0, this.width - x, this.height);
  ...
  public sliceVertically(
    ...
    //
    const topSlice = this.createCrop(0, 0, this.width, y);
    const bottomSlice = this.createCrop(0, y, this.width, this.height - y);
