...
export type ScreenMatrixPixelSurroundings = string;

//
// ─── CONSTANTS ──────────────────────────────────────────────────────────────────
//

const PIXEL_ARRAY_SIZE = 3;
const PIXEL_LEFT_INFO_OFFSET = 0;
const PIXEL_CHARACTER_OFFSET = 1;
const PIXEL_RIGHT_INFO_OFFSET = 2;
...
> {
  ...
  constructor(width: number, height: number, leftInfo: string) {
    const matrixSize = width * PIXEL_ARRAY_SIZE * height;

    this.#width = width;
    this.#height = height;
    this.#matrix = new Array<string>(matrixSize);

    for (let index = 0; index < matrixSize; index += PIXEL_ARRAY_SIZE) {
      this.#matrix[index + PIXEL_LEFT_INFO_OFFSET] = leftInfo;
      this.#matrix[index + PIXEL_CHARACTER_OFFSET] = WHITE_SPACE_CHARACTER;
      this.#matrix[index + PIXEL_RIGHT_INFO_OFFSET] = EMPTY_STRING;
    }
  }
  ...
  public writeChar(x: number, y: number, text: string): void {
    const index = this.computeIndex(x, y);
    this.#matrix[index + PIXEL_CHARACTER_OFFSET] = text;
  }
  ...
  public read(x: number, y: number): ScreenMatrixPixel {
    ...
    const index = this.computeIndex(x, y);
    const leftStylingInfo = this.#matrix[index + PIXEL_LEFT_INFO_OFFSET];
    const char = this.#matrix[index + PIXEL_CHARACTER_OFFSET];
    const rightStylingInfo = this.#matrix[index + PIXEL_RIGHT_INFO_OFFSET];
  ...
  public *iterateOnRow(row: number): Generator<ScreenMatrixPixel> {
    const startingIndex = this.#width * PIXEL_ARRAY_SIZE * row;
    const endingIndex = this.#width * PIXEL_ARRAY_SIZE * (row + 1);
  ...
  public getWholeStyledRow(
    ...
    //
    const EMPTY_STYLE = styler.renderLeftStylingInfo(styler.defaultStyle);
    let line = EMPTY_STRING;
    let previousLeftInfo = EMPTY_STYLE;
    const closingTagStack = new Array<string>();

    function closeWithRightInfoIfPresent() {
      const previousRightInfo = closingTagStack.pop();
      if (previousRightInfo) {
        line += previousRightInfo;
      }
    }

    function checkAndAddNewLeftInfo(
      leftStylingInfo: string,
    ...
    for (
      const [leftStylingInfo, character, rightStylingInfo] of this.iterateOnRow(
        row,
      )
    ) {
  ...
  public getWholePlainTextRow(row: number): string {
    let line = EMPTY_STRING;
    const startingIndex = this.#width * 2 * row;
    const endingIndex = this.#width * 2 * (row + 1);
  ...
  public renderPlainTextForm(): string {
    const lines = new Array<string>(this.#height);
    for (let row = 0; row < this.#height; row++) {
      lines[row] = this.getWholePlainTextRow(row);
    }
  ...
  public renderStyledForm(
    styler: StyleRendererProtocol<ColorType, EnvironmentStyleSettings>,
  ): string {
    const lines = new Array<string>(this.#height);

    for (let row = 0; row < this.#height; row++) {
      lines[row] = this.getWholeStyledRow(row, styler);
    }
