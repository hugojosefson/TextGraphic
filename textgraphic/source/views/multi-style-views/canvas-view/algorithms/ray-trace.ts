...
export function rayTraceScreenPixel<
...
>(
  ...
  for (const profile of canvas.getChildren()) {
    const horizontalBoundary = (xQuery >= profile.x) &&
      (xQuery < profile.x + profile.child.width);
    const verticalBoundary = (yQuery >= profile.y) &&
      (yQuery < profile.y + profile.child.height);

    if (horizontalBoundary && verticalBoundary) {
      if (result) {
        if (profile.zIndex > result.zIndex) {
          if (profile.child.transparent) {
            const [_, character] = profile.child.getCharAtRelativePosition(
              profile.x,
              profile.y,
              x,
              y,
