...
function styleOutput(output: string) {
  ...
  const styledLines = new Array<string>(lines.length);
...
function styleViewProtocolForOutputPrint(
  view: TextGraphic.ViewProtocol<any, any, any>,
) {
  const ANSI_DIM = TextGraphic.Environments.ANSITerminal.EscapeSequences.Dim;
  const ANSI_RESET =
    TextGraphic.Environments.ANSITerminal.EscapeSequences.Reset;
  const lines = new Array<string>(view.height + 2);

  // box top
  lines[0] = NORMAL_LEFT_SPACING +
    ANSI_DIM + "┌" + "─".repeat(view.width) + "┐" + ANSI_RESET;

  // middle lines
  const styledViewLines = view.styledForm.split("\n");
  for (let row = 0; row < view.height; row++) {
    const startOfLine = (row === view.baseline
      ? BASELINE_INDICATOR_LEFT_SPACING
      : NORMAL_LEFT_SPACING);
    const middleOfLine = (ANSI_DIM + "│" + ANSI_RESET +
      styledViewLines[row]! +
      ANSI_DIM + "│" + ANSI_RESET);
    const endOfLine = (row === view.baseline
      ? " /" + view.constructor.name
      : "");
    lines[row + 1] = startOfLine + middleOfLine + endOfLine;
  }

  // box bottom
  lines[view.height + 1] = NORMAL_LEFT_SPACING +
    ANSI_DIM + "└" + "─".repeat(view.width) + "┘" + ANSI_RESET;
...
export function writer(output: any, $: StyleRenderer) {
  ...
  if (typeof output === "object") {
    if (
      "styledForm" in output && "styleRenderer" in output &&
      output.styleRenderer === $
    ) {
